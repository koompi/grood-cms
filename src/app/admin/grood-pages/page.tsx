...
export default function GroodPagesAdminPage() {
  const { data: session, status } = useSession()
  const [pages, setPages] = useState<GroodPage[]>([])
  const [filteredPages, setFilteredPages] = useState<GroodPage[]>([])
  ...
  const fetchPages = async () => {
    try {
      const response = await fetch('/api/admin/grood-pages')
      if (response.ok) {
  ...
  const handleDelete = async (pageId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/grood-pages/${pageId}`, {
        method: 'DELETE',
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Grood Pages</h1>
          <p className="text-gray-500 text-sm mt-1">
            Build custom pages with blocks
          </p>
        </div>
        <Link href="/admin/grood-pages/new">
            ...
            {filteredPages.map((page) => (
              ...
              >
                <div className="flex items-start gap-4">
                  {/* Icon */}
                  <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-lg shadow-violet-500/20">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <Badge className="text-xs" variant="outline">
                          {Array.isArray(page.blocks) ? page.blocks.length : 0} blocks
                        </Badge>
                  ...
                  <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    {page.status === 'PUBLISHED' && (
                      <Button variant="ghost" size="sm" asChild className="h-8 w-8 p-0">
                        <a href={`/p/${page.slug}`} target="_blank" title="View page">
                          <ExternalLink className="h-4 w-4 text-gray-500" />
                        </a>
                      </Button>
                    )}
                    <Button variant="ghost" size="sm" asChild className="h-8 w-8 p-0">
                      <Link href={`/admin/grood-pages/${page.id}/preview`} title="Preview">
                        <Eye className="h-4 w-4 text-gray-500" />
                      </Link>
                    </Button>
                    <Button variant="ghost" size="sm" asChild className="h-8 w-8 p-0">
                      <Link href={`/admin/grood-pages/${page.id}`} title="Edit page">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem asChild>
                          <Link href={`/admin/grood-pages/${page.id}`}>
