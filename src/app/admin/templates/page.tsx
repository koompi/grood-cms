      ...
      <div className="flex flex-col sm:flex-row gap-3 py-3 border-b border-gray-200">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            ...
            className="pl-10 h-9 bg-white border-gray-200 focus:border-[#fdc501] focus:ring-[#fdc501]"
          />
        </div>
      ...
      {filteredTemplates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTemplates.map((template) => (
            <Card key={template.id} className="overflow-hidden hover:shadow-lg transition-shadow group border-gray-200 hover:border-[#fdc501]/50">
              {/* Thumbnail */}
              <div className="aspect-video bg-gradient-to-br from-[#fdc501]/10 to-[#fdc501]/5 relative">
                {template.thumbnail ? (
                  <img 
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Layout className="h-12 w-12 text-[#fdc501]/40" />
                  </div>
                )}
                ...
                <div className="absolute bottom-3 left-3">
                  <Badge className="bg-[#1a1a1a]/80 text-white hover:bg-[#1a1a1a]/90 text-xs">
                    {Array.isArray(template.blocks) ? template.blocks.length : 0} blocks
                  </Badge>
                </div>
              </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <Link 
                    ...
                    className="font-semibold text-gray-900 hover:text-[#fdc501] transition-colors"
                  >
                    {template.name}
                  </Link>
                  <Badge variant="outline" className="text-xs shrink-0">
                    <FileText className="h-3 w-3 mr-1" />
                    {template._count.pages} page{template._count.pages !== 1 ? 's' : ''}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-gray-400">
                  <span className="font-mono bg-gray-50 px-1.5 py-0.5 rounded">/{template.slug}</span>
