...
import { Star, Shield, Battery, Bike } from 'lucide-react'
import { SetPageContext, InlineEditText, EditableSection, InlineEditImage, InlineRichText } from '@/components/admin'
...
interface ProductPageClientProps {
  product: {
    ...
    reviews: { id: string; rating: number; name: string; title: string | null; content: string }[]
  }
  relatedProducts: {
    id: string
    name: string
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            {/* Product Images */}
            <EditableSection
              editUrl={`/admin/content/products/${product.id}/edit?tab=images`}
              title="Product Images"
              description="Main image and gallery"
            >
              <div>
                <div className="h-96 bg-gray-200 rounded-lg mb-4 flex items-center justify-center">
                  {product.featuredImage ? (
                    <InlineEditImage
                      ...
                      alt={product.name}
                      onSave={async (url) => handleSaveField('featuredImage', url)}
                      fill
                      className="object-cover rounded-lg"
                    />
                  ) : (
                    <span className="text-gray-400 text-2xl font-bold">GROOD</span>
                  )}
                </div>
            ...
            <div>
              ...
              <InlineEditText
                value={product.name}
                onSave={async (val) => handleSaveField('name', val)}
                as="h1"
              ...
              <div className="flex items-center mb-4">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`h-5 w-5 ${
                      i < Math.floor(Number(calculateRating(product.reviews)))
              ...
              <div className="flex items-baseline mb-6">
                <InlineEditText
                  value={`$${product.price}`}
                  onSave={async (val) => {
                    const price = parseFloat(val.replace(/[^0-9.]/g, ''))
                    if (!isNaN(price)) {
              ...
              <InlineEditText
                value={product.shortDescription || 'Premium electric bike designed for your lifestyle.'}
                onSave={async (val) => handleSaveField('shortDescription', val)}
                as="p"
          ...
          <div className="mt-16">
            ...
            <div className="py-8">
              ...
              <div className="prose prose-lg max-w-none text-gray-600">
                <InlineRichText
                  ...
                  onSave={async (content) => handleSaveField('description', content)}
            ...
            {product.specifications && (
              <>
                <Separator />
                <EditableSection
                  editUrl={`/admin/content/products/${product.id}/edit?tab=specs`}
              ...
              {relatedProducts.map((relatedProduct) => (
                <Link key={relatedProduct.id} href={`/products/${relatedProduct.slug}`} className="group">
                  <Card className="overflow-hidden border-none shadow-md rounded-2xl hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                    ...
                    <CardContent className="p-6">
                      <h3 className="font-semibold mb-2 group-hover:text-[#fdc501] transition-colors">
                        {relatedProduct.name}
                      </h3>
